...
# itsxpress <- "for i in ./seqs/*R1_001*.fastq.gz; do itsxpress --fastq $i  --outfile $i.FungalITS1.fastq.gz --region ITS1 --taxa Fungi -s --threads 4 --log $i.ITSx.log; done"
...
fns <- sort(list.files(file.path(path), full.names = TRUE, pattern = "fastq.gz.FungalITS1.fastq.gz"))

sample.names <- unlist(map(strsplit(basename(fns), "-"), 1)) 
sample.names
...
# IMPORT METADATA ####
meta = as.data.frame(read_delim("./metadata/metadata.csv",delim = ","))
row.names(meta) <- as.character(meta$LibraryID)
...
# ASSIGN TAXONOMY ####
taxa <- assignTaxonomy(seqtab.nochim, "./taxonomy/UNITE_Euk_2020-02-04_non-dev.fasta.gz", multithread=4,verbose=TRUE)
